...
func _refresh_plugin_details() -> void:
	for enabled_plugin in ProjectSettings.get_setting("editor_plugins/enabled"):
		if enabled_plugin.contains(MaaacksInputRemappingPlugin.get_settings_path()):
...
func _open_check_plugin_version() -> void:
	if check_version_scene_path.is_empty():
		...
		return
	if ProjectSettings.get_setting(MaaacksInputRemappingPlugin.get_settings_path() + "disable_update_check", false):
...
func _refresh_copy_and_delete_examples() -> void:
	var examples_path = MaaacksInputRemappingPlugin.instance.get_plugin_examples_path()
	if MaaacksInputRemappingPlugin.get_copy_path() != examples_path:
...
func _on_update_button_pressed():
	if ProjectSettings.get_setting(MaaacksInputRemappingPlugin.get_settings_path() + "disable_update_check", false):
		ProjectSettings.set_setting(MaaacksInputRemappingPlugin.get_settings_path() + "disable_update_check", false)
		_open_check_plugin_version()
		return
	else:
		tree_exited.connect(func(): MaaacksInputRemappingPlugin.instance.open_update_plugin())
		queue_free()

func _on_copy_button_pressed():
	tree_exited.connect(func(): MaaacksInputRemappingPlugin.instance.open_copy_and_edit_dialog())
	queue_free()

func _on_delete_button_pressed():
	tree_exited.connect(func(): MaaacksInputRemappingPlugin.instance.open_delete_examples_short_confirmation_dialog())
	queue_free()

func _on_add_input_icons_button_pressed():
	tree_exited.connect(func(): MaaacksInputRemappingPlugin.instance.open_input_icons_dialog())
	queue_free()
